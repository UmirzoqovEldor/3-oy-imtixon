...
function renderPost(posts) {
  ...
  for (let i = 0; i < posts.length; i++) {
    const elPost = posts[i];

    Cards.innerHTML += `
    <div class="card w-25 mt-3 ms-3 col-4">
    <p class="card-postId text-center"> ${elPost.postId} </p>
	<p class="card-id text-center">${elPost.id}</p>
    <div class="card-body">
      <h4 class="card-name text-center">${elPost.name}</h4>
      <p class="card-email text-center">${elPost.email}</p>
      <h6 class="card-body text-center">${elPost.body}</h6>
	  <button data-id="${elPost.id}" class="btn btn-secondary text-center">Delete</button>
    </div></div>`;
  }
}
...
elSelect.addEventListener("change", () => {
 ...
 let filterSlect = [];
  for (let i = 0; i < posts.length; i++) {
    const select = posts[i];
    const elId = select.postId;

    if(elId == type){
      console.log(elId);
  
      filterSlect.push(posts[i]) 
    }
    
  }
  
  renderPost(filterSlect); 
});
// search  qismi
...
elSearch.addEventListener("input", (evt) => {
  evt.preventDefault();
  searchId = [];
  let elInput = elSearch.value;
  posts.forEach((posts) => {
    if (posts.name.includes(elInput)) {
      searchId.push(posts);
    }
  });
  renderPost(searchId);
